...
| 指令 | 适用场景 | 单条/批量 | 需要讨论组 |
|------|----------|-----------|-----------|
| `/forward_screenshot` | 单条视频 → 封面图帖 + 评论区视频 | **仅单条** | 是 |
| `/forward_multi` | 批量视频 → 截图模式（单图或多图） | **批量** | 是 |
| `/forward_album` | 批量视频 → 合并为媒体组 | **批量** | 否 |
| `/forward_clone` | 克隆已有"封面图+评论区视频"结构的频道 | **批量** | 是 |

> `/forward_screenshot` **只处理单条视频消息**，范围内的多条视频会逐条各自生成一个独立帖子。如需批量处理，请用 `/forward_multi`。
...
将**单条**视频消息以"封面图发主帖、视频发评论区"的形式转发到目标频道。

> ⚠️ **仅支持视频消息**，范围内的文字、图片等非视频消息会直接普通转发。每条视频生成一个独立帖子。如需批量处理多条视频，请改用 `/forward_multi`。
