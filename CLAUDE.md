...
### 8. 相册帖的评论区锚点（`get_discussion_message` MSG_ID_INVALID → 内容误入频道）

向频道发**相册(媒体组)主帖**后，要往其评论区发内容，需先 `get_discussion_message(频道, 帖id)` 拿讨论区锚点、再 reply 到锚点。三个叠加的坑：

- **A 锚点不是第一条**：相册只有其中**一条**消息是讨论锚点，`send_media_group` 返回的 `sent[0]` **不一定**是它，用错就 `MSG_ID_INVALID`——即便评论区确实存在（能手动点开留言栏）。小相册第一条恰好是锚点所以正常，大相册可能不是。修复（`_get_discussion_message_retry`）：首次失败即用 `get_media_group` 取整组消息 id 逐条尝试、命中锚点为止，外加少量重试兜底偶发同步延迟。
- **B 静默兜底污染频道**：评论区发送的 `except` 一失败就改发**主频道**，且曾是**静默**的 → 表现为"源 1 条消息在新频道变 2 条、视频媒体组掉在频道时间线、无错误日志、bot 还以为成功"。现各兜底处已补 `logger.warning`。
- **C 单批上限 10**：`send_media_group` 一次最多 10 个；评论区用 `_send_media_batched(..., 10, ...)` 自动分批，避免 >10 报错被兜底。
