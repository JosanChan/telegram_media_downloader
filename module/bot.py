...
async def send_help_str(client: pyrogram.Client, chat_id):
    ...
    msg = (
        ...
        f"/forward_screenshot - {_t('Forward single video: photo post + video in comments (one video only)')}\n"
        f"/forward_multi - {_t('Batch forward multiple videos as screenshot mode (single or multi thumb)')}\n"
        f"/forward_album - {_t('Merge multiple videos into album')}\n"
        f"/forward_clone - {_t('Clone channel preserving discussion structure (photo post + comment videos)')}\n"
    ...
    if not node.has_protected_content:
        try:
            ...
            async for item in get_chat_history_v2(  # type: ignore
                ...
                logger.debug(f"[forward_message_impl] yielded msg {item.id}")
